  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ border: '2px solid #ef4444', borderRadius: 14, padding: 16, marginBottom: 18, background: '#fff' }}>
        ...
        {error ? <div style={{ marginTop: 10, color: '#b91c1c', fontSize: 13 }}>오류: {error}</div> : null}
      </div>
      {loading ? (
        ...
        <div style={{ border: '1px solid #eee', borderRadius: 14, overflow: 'hidden', background: '#fff' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((it) => (
                <tr key={it.id}>
                  <td style={tdStyle}>{it.provider_id ? (<Link href={`/provider/${it.provider_id}`} style={{ fontWeight: 700 }}>{it.provider_name}</Link>) : (it.provider_name || '-')}</td>
                  <td style={tdStyle}>{it.provider_region || '-'}</td>
                  <td style={tdStyle}>{it.service_name || '-'}</td>
                  <td style={{ ...tdStyle, textAlign: 'center', fontWeight: 800 }}>{typeof it.price === 'number' ? `${it.price.toLocaleString()}원` : '-'}</td>
                  <td style={tdStyle}>{it.note || '-'}</td>
                  <td style={tdStyle}>{it.source_url ? (<a href={it.source_url} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', fontWeight: 600, textDecoration: 'underline' }}>공식 출처</a>) : ('-')}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
// --- 2. 메인 페이지 (Suspense 래핑) ---
export default function ResultsPage() {
  return (
    <Suspense fallback={<div style={{ padding: 48, textAlign: 'center', color: '#6b7280' }}>결과를 불러오는 중입니다...</div>}>
      <ResultsContent />
    </Suspense>
  );
}