...
/** ---- 간단 CSV 미리보기 파서(따옴표 지원, 상위행만 표시용) ---- */
...
export default function BulkPricesPage() {
  ...
  function downloadTemplate() {
    const header = 'provider_id,service_id,price,unit,note,source_url,updated_at,last_checked_at\n';
    const blob = new Blob([header], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'prices_template.csv'; a.click();
  ...
  async function onSubmit() {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await fetch('/api/admin/prices/bulk', { method: 'POST', body: fd });
  ...
  return (
    <main style={S.wrap}>
      <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 10 }}>가격 대량등록 (CSV)</h1>
      ...
      <div style={{ ...S.card, marginBottom: 14 }}>
        ...
        <p style={{ ...S.note, marginTop: 10 }}>
          * 헤더 포함 CSV. 지원 컬럼: <code>provider_id, service_id, price, unit, note, source_url, updated_at, last_checked_at</code><br/>
          * 쉼표 포함 값은 따옴표로 감싸세요. 예: <code>"메모, 콤마 있음"</code>
        </p>
      </div>
