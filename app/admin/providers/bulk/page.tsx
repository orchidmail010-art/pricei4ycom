...
export default function BulkProvidersPage() {
  ...
  function downloadTemplate() {
    // 스키마: providers(id, name, addr, region, phone, lat, lng, updated_at)
    const header = 'name,addr,region,phone,lat,lng,updated_at\n';
    const blob = new Blob([header], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'providers_template.csv'; a.click();
  ...
  async function onSubmit() {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await fetch('/api/admin/providers/bulk', { method: 'POST', body: fd });
  ...
  return (
    <main style={S.wrap}>
      <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 10 }}>병원 대량등록 (CSV)</h1>
      ...
      <div style={{ ...S.card, marginBottom: 14 }}>
        ...
        <p style={{ ...S.note, marginTop: 10 }}>
          * 헤더 포함 CSV. 기본 컬럼: <code>name,addr,region,phone,lat,lng,updated_at</code><br/>
          * <b>이름+주소</b>에 유니크 제약이 있다면 중복 행은 실패할 수 있어요.
        </p>
      </div>
